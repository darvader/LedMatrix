...
void setupWifiUpdate()
{
  ...
  for (int i = 0; i < n; ++i)
  {
    ...
    if (WiFi.SSID(i) == christian)
    {
      WiFi.begin(christian, passwordChristian); //trying to connect the modem
      found = true;
      break;
    }
  ...
  if (!found) {
    ...
    Serial.println(WiFi.softAP(ledPushButtons, passwordLedPB, 1, false, 20) ? "Ready" : "Failed!");
  }
